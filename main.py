...
async def on_ready():
    ...
    print("🚀 ULTIMATE OWO-POKEMON ENGINE READY ON TERMUX")
...
async def help_menu(ctx):
    emb = discord.Embed(title="📜 SYCOIZZ ULTIMATE SYSTEM", color=0x00abc9)
...
async def show_profile(ctx, target: discord.User = None):
    ...
    p = get_player(u.id)
    if p["has_starter"] == 0:
        await ctx.send("❌ | Unregistered user profile profile configuration."); return
    ...
    active_pet = p["active_team"][0] if p["active_team"] else "None"
...
async def execute_battle_loop(ctx, p1_id, p1_pet, p2_id, p2_pet, is_pvp=False):
    ...
    msg = await ctx.send(f"⚔️ **MATCH START:** `{p1_pet}` (HP: {hp1}) vs `{p2_pet}` (HP: {hp2})")
    ...
    while hp1 > 0 and hp2 > 0:
        ...
        try:
            ...
            hp2 -= dmg1; if hp2 < 0: hp2 = 0
            await ctx.send(f"{ANIMATIONS.get(mv1, '💥')} -> Dealt `{dmg1}` damage!")
        except asyncio.TimeoutError: await ctx.send("⏱️ | Inactivity default swipe."); hp1=0; break
        ...
        if is_pvp:
            ...
            except asyncio.TimeoutError: hp2=0; break
        else:
            mv2 = random.choice(d2["moves"])
            dmg2 = get_damage(mv2, 1)
            
        hp1 -= dmg2; if hp1 < 0: hp1 = 0
...
async def pvp_battle_challenge(ctx, target: discord.User):
    ...
    hp1, hp2 = await execute_battle_loop(ctx, ctx.author.id, p1["active_team"][0], target.id, p2["active_team"][0], is_pvp=True)
    if hp1 > 0: await ctx.send(f"🏆 **PRO-MATCH CONCLUDED:** **{ctx.author.name}** won the prestige arena fight!")
    else: await ctx.send(f"🏆 **PRO-MATCH CONCLUDED:** **{target.name}** claimed absolute supremacy over grid tracks!")

async def main():
    async with bot: await bot.start(os.environ.get("DISCORD_TOKEN"))
